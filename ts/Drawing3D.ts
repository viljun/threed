...
export class Drawing3D {
    ...
    boundaries3D(): Boundaries3D {
        ...
        const first_point_3d = this.paths_3d[0]?.points_3d[0];
    ...
    center(): Point3D {
        ...
        return new Point3D(
            (boundaries_3d.min.x + boundaries_3d.max.x) / 2,
            (boundaries_3d.min.y + boundaries_3d.max.y) / 2,
            (boundaries_3d.min.z + boundaries_3d.max.z) / 2,
        );
    }

    move(amount_3d: Point3D): Drawing3D {
        const moved_paths_3d: Path3D[] = [];
        this.paths_3d.forEach((path_3d) => {
